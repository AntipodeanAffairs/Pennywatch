...
PennyWatch counts public statements by Senator the Hon. Penny Wong, Australian Minister for Foreign Affairs, directed **critically or positively** at named foreign countries, governments, or named foreign ministers/officials. Statements are drawn from two sources back to her swearing-in on **1 June 2022**:

- Her official X account, [@SenatorWong](https://x.com/SenatorWong).
- Her ministerial media releases at [foreignminister.gov.au/minister/penny-wong](https://www.foreignminister.gov.au/minister/penny-wong/media-releases).

Each entry in the dataset carries a `source` field (`x` or `dfat`) and, for joint statements, a `joint_with` list of co-signers. The classifier rules apply identically to both sources.
...
- **v1.5.2** (2026-06-06) — **DFAT proxy via Cloudflare Worker.** The first v1.5.0/v1.5.1 deployments confirmed that foreignminister.gov.au's WAF silently blocks connections from GitHub Actions runner IP ranges — the TCP handshake completes but the response never arrives, producing read-timeouts on every request. Solution: a small Cloudflare Worker hosted on Antipodean Affairs's existing Cloudflare account fronts the DFAT site for the scraper. The Worker accepts authenticated requests (shared secret in X-PennyWatch-Auth header), allowlists only Senator Wong's ministerial subtree on foreignminister.gov.au, fetches the requested page from Cloudflare's edge network (which is not on the WAF blocklist), and returns the response. Edge cache is 5 minutes per URL so repeated requests don't burden the upstream. fetch_dfat.py routes through the proxy when the `DFAT_PROXY_URL` and `DFAT_PROXY_SECRET` environment variables are present; without them it falls back to direct fetch (useful for local development). The proxy is also documented in source-available form so any reader can verify what it does.
- **v1.5.1** (2026-06-06) — Two fixes after the first live v1.5.0 deploy returned 0 DFAT records. (a) **User-Agent**: the deliberately-honest "Mozilla/5.0 (compatible; PennyWatch; +URL)" UA was being silently blocked by the foreignminister.gov.au WAF — only browser-shaped UAs return content. Switched to a Firefox-shaped UA with a `PennyWatch/1.0` token appended at the end, which passes the WAF while still identifying the project for sysadmins inspecting logs. (b) **First-run incremental bug**: when no DFAT records existed yet in the dataset, the incremental-start-time logic was using the latest X tweet's date as the reference point, so the first DFAT run only fetched the past ~5 days. Now DFAT has its own incremental clock: if there are no existing DFAT records, the run does a full backfill from 1 June 2022 regardless of how many X records are present.
- **v1.5.0** (2026-06-06) — **Dual-source dataset.** PennyWatch now combines @SenatorWong tweets with Wong's official ministerial media releases scraped from foreignminister.gov.au/minister/penny-wong/media-releases. Each entry in the dataset carries a `source` field (`x` or `dfat`); joint statements (releases co-signed by other ministers — PM, DPM, G7 counterparts, Quad partners, AUKUS partners, etc.) carry a `joint_with` list naming the co-signers. The classifier and target list are unchanged — exactly the same rules apply to both sources. The DFAT scraper paginates politely (0.5s between requests, descriptive User-Agent) and stops once it reaches releases predating 1 June 2022. Joint statements are counted in the headline totals because Wong's name is on them as a signatory, but every such statement carries the metadata so readers can apply their own weighting. The table has a new Source filter; each row carries an X or DFAT badge. Methodology version published in the header tells you which version produced any given count.
